...
def test_get_timeline_by_match_id():
    client = RiotClient(os.environ.get("API_KEY"), REGION)
    result = client.get_timeline_by_match_id(VALID_MATCH_ID)
    assert 'frames' in result


def test_bad_get_timeline_by_match_id():
    client = RiotClient(os.environ.get("API_KEY"), REGION)
    with pytest.raises(RiotClient.APIException):
        client.get_timeline_by_match_id(INVALID_MATCH_ID)
